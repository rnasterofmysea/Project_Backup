...
app.get("/bringTimelineData", (req, res) => {
    ...
    const sql = "SELECT * FROM timeline_table WHERE (timeline_time BETWEEN date_format(?, %T) AND date_format(?,%T)) AND (timeline_job = ?)";
    const params = req.query;
    console.log("timeline 가져오는 부분 :::"+params);
    connection.query(sql, params.start, params.end, params.job, (err, rows, fields) => {
        if (!err) {
            res.send(rows)
        } else {
            res.send(err)
