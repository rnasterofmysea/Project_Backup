...
app.get("/SignIn", (req, res) => {
    const params = req.query;
    connection.query("INSERT INTO user_table(user_id ,user_pw, user_type)", [params.id,
    params.pw, "user"], function (err, rows, fields) {
...
app.post("/addJobData", (req, res) => {
    ...
    connection.query("INSERT INTO job_table(job,location,pay) values (? , ?, ?) WHRE user_id = ?", [data.job, data.location, data.pay, data.user_id],
        function (err, rows, fielda) {
            if (err) {
...
app.get("/getMonthSalary", (req, res) => {
    ...
    const sql = "SELECT user_id FROM schedule WHERE job = ? AND YEAR(start) = year(NOW()) AND MONTH(start) = MONTH(NOW())";
